...
RUN apt-get update && \
    ...
    cd /usr/local/src && wget ftp://ftp.unidata.ucar.edu/pub/udunits/udunits-2.2.21.tar.gz && \
    tar zxf udunits-2.2.21.tar.gz && cd udunits-2.2.21 && ./configure && make && make install && \
